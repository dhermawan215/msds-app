var Index = (function () {
    ...
    var table;
    var aSelected = [];
    const nameOfModule = "sample-source";
    ...
    var handleDataTabelUser = function () {
        table = $("#tbl-" + nameOfModule).DataTable({
            ...
            serverSide: true,
            ajax: {
                url: url + "/rnd/sample-source/list",
                type: "POST",
                data: {
                    _token: csrf_token,
                },
    ...
    var handleAdd = function () {
        $("#form-add-" + nameOfModule).submit(function (e) {
            ...
            if (confirm("Are you sure?")) {
                $.ajax({
                    url: `${url}/rnd/sample-source/save`,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function (responses) {
                        ...
                        setTimeout(() => {
                            $("#modal-add-" + nameOfModule).modal("toggle");
    ...
    var handleUpdate = function (dataEdit) {
        $("#form-edit-" + nameOfModule).submit(function (e) {
            ...
            if (confirm("Are you sure?")) {
                $.ajax({
                    url: `${url}/rnd/sample-source/update`,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function (responses) {
                        toastr.success(responses.message);
                        setTimeout(() => {
                            $("#modal-edit-" + nameOfModule).modal("toggle");
                            $("#form-edit-" + nameOfModule).trigger("reset");
                            table.ajax.reload();
                        }, 2500);
                    },
                    error: function (response) {
