var Index = (function () {
    ...
    var table;
    var aSelected = [];
    const pageName = "storage-precautionary";
    var handleDataTabelUser = function () {
        table = $("#tbl-" + pageName).DataTable({
            ...
            serverSide: true,
            ajax: {
                url: url + "/storage-precautionary/list",
                type: "POST",
                data: {
                    _token: csrf_token,
                },
        ...
        $("#btn-delete").click(function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: url + "/storage-precautionary/delete",
                        data: {
                            _token: csrf_token,
                            dValue: aSelected,
                        },
    ...
    var handleSubmit = function () {
        $("#form-add-" + pageName).submit(function (e) {
            ...
            if (confirm("is it correct?")) {
                $.ajax({
                    url: url + "/storage-precautionary/save",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
    ...
    var handleDetail = function () {
        $(document).on("click", ".btn-detail", function (e) {
            e.preventDefault();
            const dlVal = $(this).data("dtl");

            $.ajax({
                url: `${url}/storage-precautionary/detail`,
                type: "POST",
                data: {
                    _token: csrf_token,
                    dl: dlVal,
                ...
                success: function (responses) {
                    ...
                    $("#code-detail").val(dataResponse.code);
                    $("#description-detail").val(dataResponse.description);
                    $("#lang-detail").val(dataResponse.language);
                },
                error: function (response) {},
            });
        });
