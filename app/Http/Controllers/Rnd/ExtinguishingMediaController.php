...
use App\Repository\ExtinguishingRepository;

class ExtinguishingMediaController extends Controller
{
    //EXTINGUISHING MEDIA  controller
    use ModulePermissions;
    protected $sysModuleName = 'extinguishing_media';
    const valuePermission = ['add', 'edit', 'delete'];
    protected static $url;
    protected $extinguishingRepo;

    public function __construct(ExtinguishingRepository $extinguishingRepository)
    {
        static::$url = \route($this->sysModuleName);
        $this->extinguishingRepo = $extinguishingRepository;
    }
    ...
    public function index()
    {
        ...
        return \view('pages.msds.extinguishing.index', ['moduleFn' => $moduleFn]);
    }
    /**
     * datatable for extinguishing media
     * @return json
     */
    public function listData(Request $request)
    {
        ...
        $query = MasterExtinguishingMedia::select('*');
        ...
        foreach ($resData as $key => $value) {
            ...
            if (in_array(static::valuePermission[1], $moduleFn)) {
                ...
                <button class="btn btn-sm btn-success btn-edit" title="edit" data-toggle="modal" data-target="#modal-edit-extinguishing" data-ed="' . base64_encode($value->id) . '"><i class="fa fa-edit" aria-hidden="true"></i></button>
                </div>';
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $this->extinguishingRepo->saveExtinguishing($data);
    ...
    {
        try {
            $extinguishing = $this->extinguishingRepo->detailExtinguishing(\base64_decode($request->de));
            $data = [
                'description' => $extinguishing->description,
                'notes' => $extinguishing->notes,
            ];
            switch ($extinguishing->language) {
                case 'EN':
                    $data['lang'] = 'Eglish';
                    $data['lang_value'] = 'EN';
                    break;
    ...
    public function update(Request $request)
    {
        ...
        try {
            $this->extinguishingRepo->updateExtinguishing($request);
    ...
    {
        try {
            $this->extinguishingRepo->deleteExtinguishing($request->dValue);
