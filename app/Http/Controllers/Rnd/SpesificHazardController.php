...
use App\Repository\SpesificHazardRepository;

class SpesificHazardController extends Controller
{
    //controller for module spesific hazard
    use ModulePermissions;
    protected $sysModuleName = 'spesific_hazard';
    const valuePermission = ['add', 'edit', 'delete'];
    protected static $url;
    protected $spesificHazardRepo;

    public function __construct(SpesificHazardRepository $spesificHazardRepository)
    {
        $this->spesificHazardRepo = $spesificHazardRepository;
    }
    ...
    public function index()
    {
        ...
        return \view('pages.msds.spesific-hazard.index', ['moduleFn' => $moduleFn]);
    }

    /**
     * datatable for spesific hazard
     * @return json
     */
    public function listData(Request $request)
    {
        ...
        $query = MasterSpesificHazard::select('*');
        ...
        foreach ($resData as $key => $value) {
            ...
            if (in_array(static::valuePermission[1], $moduleFn)) {
                ...
                <button class="btn btn-sm btn-success btn-edit" title="edit" data-toggle="modal" data-target="#modal-edit-spesific-hazard" data-ed="' . base64_encode($value->id) . '"><i class="fa fa-edit" aria-hidden="true"></i></button>
                </div>';
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $this->spesificHazardRepo->saveSH($data);
    ...
    {
        try {
            $spesificHazard = $this->spesificHazardRepo->detailSH(\base64_decode($request->de));
            $data = [
                'description' => $spesificHazard->description,
                'notes' => $spesificHazard->notes,
            ];
            switch ($spesificHazard->language) {
                case 'EN':
                    $data['lang'] = 'Eglish';
                    $data['lang_value'] = 'EN';
                    break;
    ...
    public function update(Request $request)
    {
        ...
        try {
            $this->spesificHazardRepo->updateSH($request);
    ...
    {
        try {
            $this->spesificHazardRepo->deleteSH($request->dValue);
